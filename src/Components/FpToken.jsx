...
function FpToken() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length == 0) {
      try {
        const response = await axios.post("http://localhost:8082/api/v1/auth/sendFpToken", formData, {
          headers: {
        ...
        if (data.toLowerCase().trim() == "Invalid Email. Register if you didn't register before".toLowerCase().trim()) {
          ...
          setSuccessMessage('Token for changing password sent Successfully');
